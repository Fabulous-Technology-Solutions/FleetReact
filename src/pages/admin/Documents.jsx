    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-5">
        {compliant.map((doc, index) => (
          <div 
            key={index} 
            className='border border-main p-4 rounded-[16px] flex flex-col justify-between h-full'
          >
            <div className='relative'>
              <img src={doc.comImg} alt="" className='rounded-md brightness-75'/>
              <div className={`absolute top-1 right-1 py-1 px-2 rounded-full text-xs ${getColor(doc.compliantCard)}`}>
                  <p className='text-white flex gap-1 items-center'><GoDotFill />{doc.compliantCard}</p>
              </div>
            </div>
            <div className='mt-3'>
